...
parser.add_argument("--checkpoint", default="check_1",
...
def create_model(inputs, targets):
    ...
    def compute_p_loss(discrim_inputs_p):
        ...
        with tf.variable_scope("layer_1_p"):
            ...
            rectified_p = tf.nn.relu(convolved_p)
        ...
        for i in range(n_layers_p):
            with tf.variable_scope("layer_%d_p" % (len(layers_p) + 1)):
                ...
                rectified_p = tf.nn.relu(normalized_p)
                layers_p.append(rectified_p)
        ...
        with tf.variable_scope("layer_%d_p" % (len(layers_p) + 1)):
            ...
            output_p = tf.nn.relu(convolved_p)
    ...
    with tf.name_scope("generator_loss"):
        ...
        gen_loss_P = tf.reduce_sum(tf.nn.l2_loss(p_output - p_target))  # 225=30*30
        # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
        gen_loss = gen_loss_GAN * a.gan_weight + gen_loss_L1 * a.l1_weight + gen_loss_P + gen_loss_L2  # *0.01 + gen_loss_P #+ gen_loss_Res   # 无残差损失
...
def main():
    ...
    # tf.summary.scalar("generator_loss_L1", model.gen_loss_L2)
    # tf.summary.scalar("generator_loss_L1", model.gen_loss_P)
